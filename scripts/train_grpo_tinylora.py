...
"""GRPO-style TinyLoRA training entrypoint for causal language models."""
...
from tinylora.data import QADatasetSpec, build_prompt, load_qa_split
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train TinyLoRA with a GRPO-style objective on a HF QA dataset.")
...
def _policy_logprob_sum_for_generated(
    model: torch.nn.Module,
    full_ids: torch.Tensor,
    prompt_len: int,
) -> torch.Tensor:
    # full_ids shape: [1, seq_len]
    logits = model(full_ids[:, :-1]).logits  # predicts token at t+1
    target = full_ids[:, 1:]
    token_logp = F.log_softmax(logits, dim=-1).gather(dim=-1, index=target.unsqueeze(-1)).squeeze(-1)
    gen_len = full_ids.shape[1] - prompt_len
    if gen_len <= 0:
        return token_logp.new_tensor(0.0)
...
def main(argv: list[str]) -> int:
    ...
    steps_per_epoch = math.ceil(len(train_data) / max(1, args.batch_prompts))
    ...
    for step_idx in range(train_state["global_step"], max_steps):
        ...
        batch_ids = order[train_state["cursor"] : train_state["cursor"] + args.batch_prompts]
        train_state["cursor"] += len(batch_ids)
        batch = [train_data[i] for i in batch_ids]

        optimizer.zero_grad(set_to_none=True)
        sample_logprob_sums: list[torch.Tensor] = []
        ...
        for ex in batch:
            prompt = build_prompt(ex.question)
            enc = tokenizer(prompt, return_tensors="pt", truncation=True, max_length=args.max_prompt_tokens)
            input_ids = enc["input_ids"].to(device)
            attention_mask = enc["attention_mask"].to(device)
            ...
            for g in range(args.group_size):
                full_ids = outputs[g : g + 1]
                generated = full_ids[0, prompt_len:]
                pred_text = tokenizer.decode(generated, skip_special_tokens=True)
                reward = exact_match_reward(pred_text, ex.answer)
                group_rewards.append(reward)
                group_logprobs.append(_policy_logprob_sum_for_generated(model, full_ids, prompt_len=prompt_len))

            mean_reward = sum(group_rewards) / max(1, len(group_rewards))
            for reward, logp in zip(group_rewards, group_logprobs):
                sample_advantages.append(reward - mean_reward)
                sample_logprob_sums.append(logp)
                rewards_acc += reward
                sample_count += 1
        ...
        adv_tensor = torch.tensor(sample_advantages, device=device, dtype=torch.float32)
        logp_tensor = torch.stack(sample_logprob_sums).to(torch.float32)
        loss = -(adv_tensor * logp_tensor).mean()
